                ...
                if (data != null && type != null)
                {
                    ...
                    try
                    {
                        ...
                        if (type == "GetProductList")
                        {
                            ...
                            List<ProductInCampaign> productInCampaignList = await productInCampaign.MySQL_GetProductList_ListAsync(Configuration.DatabaseMode);
                            log.LogInformation($"Function Found {className}");
                            return new OkObjectResult(Functions.ObjectToJsonString(productInCampaignList));
                        }

                        if (type == "GetDonatedProductForShipping")
                        {
                            className = "Get Product List";
                            ...
                            List<ProductDonated> productInCampaignList = await productDonated.MySQL_GetDonatedProductForShipping_ListAsync(Configuration.DatabaseMode);
                            log.LogInformation($"Function Found {className}");
                            return new OkObjectResult(Functions.ObjectToJsonString(productInCampaignList));
                        }
                    ...
                    try 
                    {
                        ...
                        switch (type)
                        {
                            case "SetNewProduct":
                                ...
                                action = await ProductInCampaign.SetNewProductAsync(Configuration.DatabaseMode);
                                break;
                            ...
                            case "SetBuyAnItem":
                                ...
                                action = await productDonated.SetBuyAnItemAsync(Configuration.DatabaseMode);
                                break;
                            case "SetProductShipping":
                                ...
                                action = await productDonated2.SetProductShippingAsync(Configuration.DatabaseMode);
                                break;
